...
uint8_t Extract_MSB(uint16_t i) { return i >> 8; }

uint8_t Extract_LSB(uint16_t i) { return (i & 0x00FF); }
...
void Flag_set_Z(uint8_t state) { Registers.F = (state << 7) | Registers.F; }

void Flag_set_N(uint8_t state) { Registers.F = (state << 6) | Registers.F; }

void Flag_set_H(uint8_t state) { Registers.F = (state << 5) | Registers.F; }

void Flag_set_C(uint8_t state) { Registers.F = (state << 4) | Registers.F; }
...
uint8_t Flag_get_Z(void) { return (Registers.F & 0x80) >> 7; }

uint8_t Flag_get_N(void) { return (Registers.F & 0x40) >> 6; }

uint8_t Flag_get_H(void) { return (Registers.F & 0x20) >> 5; }

uint8_t Flag_get_C(void) { return (Registers.F & 0x10) >> 4; }